...
# Prompt for the file paths of the datasets
products_path = input("Enter the path for the product dataset (e.g., '/path/to/products.json'): ")
reviews_path = input("Enter the path for the reviews dataset (e.g., '/path/to/reviews.json'): ")

# Load the product dataset
df_products = pd.read_json(products_path)

# Load the review dataset
df_reviews = pd.read_json(reviews_path)

# Merge the datasets based on the common key (assuming 'serial_number' is the common key)
df = pd.merge(df_products, df_reviews, on='serial_number', how='left')
...
# Calculate sentiment scores (assumes 'review_text' is the column containing reviews in the df_reviews)
...
features = ['log_rank_01', 'log_rank_02', 'price', 'star_rating', 'total_rating', 'scraping_time_minutes', 
            ...
            'scraping_weekday', 'price_to_rating_ratio', 'log_rank_interaction', 'sentiment_score']
...
with open('data2/sales_predictions2.json', 'w') as json_file:
    json.dump(json_data, json_file, indent=4)

# Save the results in CSV format for easy access
df[['serial_number', 'title', 'category_01', 'best_sellers_rank_01_no', 'predicted_sales']].to_csv('data2/sales_predictions2.csv', index=False)